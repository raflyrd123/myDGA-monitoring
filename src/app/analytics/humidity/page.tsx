...
export default function HumidityAnalyticsPage() {
  ...
  const fetchData = async () => {
    ...
    if (allRawData.length > 0) {
      // Data Sanitization: Filter rational humidity values (0% - 100%)
  ...
  return (
    <div className="p-10 text-[#2D365E] min-h-screen bg-[#f8fafc]">
      ...
      <div className="flex justify-between items-center mb-10">
        <h1 className="text-4xl font-black uppercase tracking-tight text-[#2D365E]">Analytics - Humidity</h1>
      ...
      <div className="grid grid-cols-1 md:grid-cols-3 gap-8 mb-10">
        ...
        <div className="bg-[#2D365E] rounded-[40px] p-8 shadow-xl flex items-center justify-between overflow-hidden relative border border-white/5">
          <div className="z-10">
            <p className="text-xs font-black text-white/40 uppercase tracking-widest mb-2">Humidity Status</p>
      ...
      <div className="bg-white rounded-[50px] p-10 shadow-2xl flex flex-col h-[600px] relative border border-gray-100/40">
        <div className="flex justify-between items-center mb-8 border-b pb-4">
          <h2 className="text-2xl font-black uppercase tracking-tighter">Humidity Trend</h2>
        </div>
        ...
        <div className="flex-grow flex items-center justify-center w-full">
          {!loading && historyData.length > 0 && settings ? (
            <ResponsiveContainer width="100%" height="100%">
              <AreaChart data={historyData}>
                ...
                {settings.humidity?.dry > 0 && (
                  <ReferenceLine y={settings.humidity.dry} stroke="#cb6060" strokeDasharray="6 6" strokeWidth={1.5} label={{ value: `DRY: ${settings.humidity.dry}%`, position: 'top', fill: '#cb6060', fontSize: 9, fontWeight: 'bold' }} />
                )}
                {settings.humidity?.wet > 0 && (
                  <ReferenceLine y={settings.humidity.wet} stroke="#2ac7c7" strokeDasharray="6 6" strokeWidth={1.5} label={{ value: `WET: ${settings.humidity.wet}%`, position: 'top', fill: '#2ac7c7', fontSize: 9, fontWeight: 'bold' }} />
                )}
            ...
            <div className="flex flex-col items-center opacity-15 select-none">
               ...
               <p className="font-black text-2xl uppercase tracking-[0.2em] text-[#2D365E]">No Humidity Data Available</p>
            </div>
          )}
        </div>
