...
import { Waves } from 'lucide-react'; // FIX: Import Waves ditambahkan di sini
...
export default function HumidityAnalyticsPage() {
  ...
  const fetchData = async () => {
    ...
    // Reset data awal agar tidak ada data "hantu" dari range sebelumnya
    setHistoryData([]);
    ...
    if (data && data.length > 0) {
      const hums = data.map(d => d.humidity_pct || 0);
  ...
  const currentStatus = getStatusInfo(stats.avg);
    ...
    <div className="p-10 text-[#2D365E]">
      ...
      <div className="grid grid-cols-1 md:grid-cols-3 gap-8 mb-10">
        ...
        <div className="bg-[#2D365E] rounded-[40px] p-8 shadow-xl flex items-center justify-between overflow-hidden relative">
          <div className="z-10">
            <p className="text-xs font-black text-white/40 uppercase tracking-widest mb-2">Current Vibe</p>
      ...
      <div className="bg-white rounded-[50px] p-10 shadow-2xl flex flex-col h-[600px] relative">
          ...
          {loading ? (
            ...
            <ResponsiveContainer width="100%" height="100%">
              <AreaChart data={historyData}>
                ...
                <YAxis axisLine={false} tickLine={false} tick={{fill: '#2D365E', fontSize: 12, fontWeight: 'bold'}} domain={[0, 100]} />
            ...
            <div className="flex flex-col items-center opacity-20">
               <Waves className="w-20 h-20 mb-4" /> {/* Icon ini yang tadi bikin error[cite: 1] */}
               <p className="font-black text-3xl uppercase tracking-[0.3em]">No Humidity Data Available</p>
            </div>
          )}
        </div>
        ...
        <div className="flex justify-center gap-10 border-t border-gray-50 pt-8 mt-4">
           ...
           <div className="flex items-center gap-2">
              ...
              <span className="text-[10px] font-black text-gray-400 uppercase tracking-widest">Normal &lt; {settings?.humidity.wet}%</span>
           </div>
           <div className="flex items-center gap-2">
