...
export async function POST(request: Request) {
  try {
    ...
    const { data, error: resendError } = await resend.emails.send({
      from: 'Koper DGA Smart Alert <alerts@resend.dev>', 
      to: emailTargets,                                 
      ...
      html: `
        <div style="font-family: -apple-system, BlinkMacSystemFont, 'Segoe UI', Roboto, sans-serif; padding: 24px; color: #2D365E; max-width: 600px; border: 1px solid #E2E8F0; border-radius: 16px; background-color: #FFFFFF;">
          <div style="display: flex; align-items: center; gap: 8px; margin-bottom: 16px;">
            <span style="background-color: #FEE2E2; color: #EF4444; padding: 4px 12px; border-radius: 9999px; font-size: 10px; font-weight: 900; letter-spacing: 0.1em; text-transform: uppercase;">
          ...
          <div style="margin-top: 24px; border-top: 1px solid #E2E8F0; padding-top: 16px; display: flex; justify-content: space-between; align-items: center;">
