...
export default function TemperatureAnalyticsPage() {
  ...
  const fetchData = async () => {
    ...
    if (allRawData.length > 0) {
      // Data Sanitization: Filter rational temperature values (0°C to 500°C)
  ...
  return (
    <div className="p-10 text-[#2D365E] min-h-screen bg-[#f8fafc]">
      ...
      <div className="flex justify-between items-center mb-10">
        <h1 className="text-4xl font-black uppercase tracking-tight text-[#2D365E]">Analytics - Temperature</h1>
      ...
      <div className="grid grid-cols-1 md:grid-cols-3 gap-8 mb-10">
        <div className="bg-white rounded-[40px] p-8 shadow-xl border border-gray-100/50">
          <p className="text-xs font-black text-gray-400 uppercase tracking-widest mb-2">Average Temperature</p>
          <h3 className="text-5xl font-black text-[#2D365E]">{stats.avg === 0 ? '--' : stats.avg.toFixed(1) + '°C'}</h3>
        </div>
        <div className="bg-white rounded-[40px] p-8 shadow-xl border border-gray-100/50">
          <p className="text-xs font-black text-gray-400 uppercase tracking-widest mb-2">Peak Temperature</p>
        ...
        <div className="bg-[#2D365E] rounded-[40px] p-8 shadow-xl flex items-center justify-between overflow-hidden relative border border-white/5">
          <div className="z-10">
            <p className="text-xs font-black text-white/40 uppercase tracking-widest mb-2">Temperature Status</p>
        ...
        <div className="flex-grow flex items-center justify-center w-full">
          {!loading && historyData.length > 0 && settings ? (
            <ResponsiveContainer width="100%" height="100%">
              <AreaChart data={historyData}>
                ...
                {settings.temp?.warn > 0 && (
                  <ReferenceLine y={settings.temp.warn} stroke="#d8db26" strokeDasharray="6 6" strokeWidth={1.5} label={{ value: `CAUTION: ${settings.temp.warn}°C`, position: 'top', fill: '#b4980a', fontSize: 9, fontWeight: 'bold' }} />
                )}
            ...
            <div className="flex flex-col items-center opacity-15 select-none">
               ...
               <p className="font-black text-2xl uppercase tracking-[0.2em] text-[#2D365E]">No Temperature Data Available</p>
            </div>
          )}
        </div>
